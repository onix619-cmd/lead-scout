...
export function generateLandingPageHTML(
  ...
  const menuCarouselHtml = galleryImages.length > 0 ? `
    <div class="mt-10 relative max-w-2xl mx-auto">
      <h3 class="text-lg font-semibold mb-3 text-center font-display" style="color:var(--primary);">Menu &amp; Gallery Photos</h3>
...
<section id="featured" class="py-20" style="background:#141414;">
  <div class="max-w-6xl mx-auto px-6">
    ...
    ${originalMenuPhotoUrl || lead.website ? `<div class="text-center mt-8"><a href="${originalMenuPhotoUrl || lead.website}" target="_blank" class="inline-block px-6 py-3 r-card font-medium border font-display" style="border-color:var(--primary); color:var(--primary);">View Full Menu Link ↗</a></div>` : ""}
    ${menuCarouselHtml}
  </div>
</section>
...
<footer class="bg-black py-12 text-sm text-neutral-400 border-t border-white/10">
  <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
    <p>&copy; ${new Date().getFullYear()} ${escapeHtml(lead.name)}. All rights restored.</p>
