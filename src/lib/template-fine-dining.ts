...
// Always shows all three platform icons in the footer, regardless of
// whether a real link was found — icons for platforms we have a link for
// are clickable and open it; the rest render dimmed and inert instead of
// disappearing, so the row always reads as "find us here" rather than
// only showing up when every platform happens to be linked.
...
<body class="antialiased">
  ...
  <footer class="pt-14 pb-8" style="background:#120E0C; color:#8A7D75;">
    <div class="max-w-6xl mx-auto px-4 md:px-8 grid sm:grid-cols-3 gap-8 text-sm">
      ...
      <div>
        ...
        <div class="mt-4">${socialIconsHtml(lead, "#E07A5F", "#8A7D75")}</div>
      </div>
    </div>
  ...
  <script>
    ...
    function fdCarouselAutoStop() {
      if (fdCarouselTimer) { clearInterval(fdCarouselTimer); fdCarouselTimer = null; }
    }
    function fdCarouselUserMove(dir) {
      fdCarouselMove(dir);
      fdCarouselAutoStart();
    }
    fdPositionSlides();
